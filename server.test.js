...
// Configure rate limiting - disable in test environment
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: process.env.NODE_ENV === 'test' ? 1000 : 100, // Very high limit in tests to avoid interference
...
// Skip MongoDB connection in test environment - use mocked models
if (process.env.NODE_ENV !== 'test') {
  const mongoose = require('mongoose');
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/piper_newsletter';
...
// Use mocked models in test environment
...
app.get('/api/user/profile', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('Available users in mock DB:', Array.from(User._users?.keys() || []));
